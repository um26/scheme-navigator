    ...
    <div className={`max-w-4xl mx-auto px-4 py-10 ${compareMode ? "pb-28" : ""}`}>
      ...
      {!hydrated || saved === null ? (
        ...
        <>
          ...
          {compareMode && showTable && compareSchemes.length >= 2 && (
            <div className="mt-4 animate-fadeIn"><CompareTable schemes={compareSchemes} /></div>
          )}
            ...
            {saved.map((scheme) => {
              ...
              return (
                <div key={scheme.id} className={`rounded-xl transition-shadow ${selected ? "ring-2 ring-saffron-dark ring-offset-2 ring-offset-khadi" : ""}`}>
                  ...
                  <button type="button" onClick={() => removeSaved(scheme.id)} className="mt-1 px-1 text-xs font-body text-muted transition-colors hover:text-red-700">{t("saved_remove")}</button>
                </div>
              );
            })}
          </div>
